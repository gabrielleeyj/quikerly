	...
	return (
		<>
								...
								<TableBody>
									{(limit > 0
										...
										>
											...
											<TableCell>{customer.userPostalCode}</TableCell>
											<TableCell>{customer.userContact}</TableCell>
				...
				{customers && (
					<TablePagination
						...
						rowsPerPageOptions={[
							...
							10,
							25,
							{ value: customers.length, label: "All" },
						]}
					/>
				)}
			</Card>
			...
			>
				<Typography
					color="secondary"
					style={{ fontSize: "1.3em", textAlign: "center", marginTop: "5vh" }}
				>
					Orders placed by{" "}
					{selectedCustomer ? selectedCustomer.userName.toUpperCase() : ""}
				</Typography>
				<Table key={2} size="medium">
					...
					{orders && selectedCustomer && (
						<TableBody>
							{orders
								.filter(
									(order) => order.userEmail === selectedCustomer.userEmail
								)
								.map((row) => (
